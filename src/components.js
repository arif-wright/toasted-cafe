  ...
  return items.map(item => `
    <article class="food-card">
      ...
      <div class="food-card-copy">
        <div class="food-card-title"><div><h3>${item.name}</h3><span class="served-with">Served with your choice of side</span></div><button class="add-btn item-trigger" data-item-id="${item.id}" aria-label="Customize ${item.name}">+</button></div>
        <p>${item.desc}</p><div class="card-bottom"><strong>${money(item.price)}</strong><button class="text-button item-trigger" data-item-id="${item.id}">Customize</button></div>
      </div>
    </article>
  `).join("");
}
  ...
  return items.map(item => `
    <article class="menu-item">
      <button class="menu-item-main item-trigger" data-item-id="${item.id}">
        <span><h3>${item.name}</h3><p>${item.desc}</p>${item.flags.length ? `<span class="menu-flags">${item.flags.map(flag => `<span>${flag}</span>`).join("")}</span>` : ""}</span>
        ${item.image ? `<img src="${item.image}" alt="" loading="lazy">` : ""}
      </button>
      <div class="menu-item-bottom"><strong>${money(item.price)}</strong><button class="menu-add item-trigger" data-item-id="${item.id}">Customize +</button></div>
    </article>
  `).join("");
}
...
export function renderModifierGroup(group, selection) {
  return `
    <fieldset class="modifier-group" data-group-id="${group.id}" ${group.showWhen ? `data-show-group="${group.showWhen.group}" data-show-value="${group.showWhen.value}" hidden` : ""}>
      <legend><span>${group.name}</span><small>${group.required ? "Required" : "Optional"}</small></legend>
...
export function renderCartLine(line) {
  ...
  return `
    <article class="cart-line" data-line-id="${line.lineId}">
      <div><h3>${line.item.name}</h3><p>${details || "As is"}</p><button class="remove-line" data-line-id="${line.lineId}">Remove</button></div>
